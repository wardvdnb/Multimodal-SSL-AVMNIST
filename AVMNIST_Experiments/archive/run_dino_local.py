...
from models.dino import MultiModalDINOLightning, UniModalDINOLightning, ImageEncoder, \
SpectrogramEncoder, SpectrogramEncoderCentral, SpectrogramEncoderLSTM, SpectrogramEncoderResidual, \
SpectrogramEncoderViT, SpectrogramEncoderMobileViT, SpectrogramEncoderResNet, SpectrogramEncoderAST
from utils.get_data import AVMNISTDinoDataModule
from datetime import datetime
from utils.plots_trials import load_all_versions, process_metrics, save_versions_to_csv, create_dir, plot_loss
...
def experiment(config, model):
    ...
    model_dir = f"{config['model']['model_dir']}/{model_name}"

    logger = CSVLogger(f"{model_dir}", name="logs")
    mode = "max" if config['model']['metric'] == 'mlp_acc' else "min"
    
    # Set up callbacks
    checkpoint = ModelCheckpoint(dirpath=model_dir, monitor=config['model']['metric'], save_top_k=1, mode=mode)
    early_stopping = EarlyStopping(monitor=config['model']['metric'], patience=3, mode=mode)
    ...
    # Save checkpoint
    trainer.save_checkpoint(f"{model_dir}/{model_name}.ckpt")
    ...
    df.to_csv(os.path.join(model_dir, f"final_results_{config['model']['name']}.csv"), index=False)
    
    # Compute and save additional results
    knn_acc, mlp_acc = compute_train_results(model, model_dir, model_name, config)
    ...
    perf_summary = {
        ...
        "best_train_loss": f"{float(trainer.callback_metrics.get('train_loss', 0)):.4f}",
        f"best_{config['model']['metric']}": f"{float(trainer.callback_metrics.get(config['model']['metric'], 0)):.4f}",
...
def compute_train_results(module, model_path, model_name, config):
    """
    Save Plot results (e.g. pca for frozen encoders) and 
    Accuracies (for downstream tasks (KNN and MLP on 10 epochs))
    """
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    pca_plot_path = os.path.join(model_path, 'pca_plots')
    confusion_matrix_path = os.path.join(model_path, 'confusion_matrix')
...
def main():
    config_path = os.path.join(os.getcwd(), 'configs', 'config_local.yaml')

    config = yaml.safe_load(open(config_path))
    model = UniModalDINOLightning(
        encoder_class = SpectrogramEncoderResNet,
        # encoder_kwargs={'pretrained': True},
        data_dir=config['data']['data_dir'], 
        dropout=config['hyperparameters']['dropout'], # NOTE: dropout for the encoder
        learning_rate = config['hyperparameters']['learning_rate'],
        projection_dim = config['hyperparameters']['projection_dim'],
        output_dim = config['hyperparameters']['output_dim'],
        momentum = config['hyperparameters']['momentum'],
