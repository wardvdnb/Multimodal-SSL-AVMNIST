...
def search_hyperparameters_unimodal(config, config_name, model_dir, encoder_class):
    ...
    try:
        ...
        if remaining_trials > 0:
            ...
            study.optimize(
                lambda trial: objective(trial, config, model_dir, encoder_class), 
                n_trials=remaining_trials, 
                n_jobs=config['optuna']['num_parallel_trials'],  # Parallel optimization still enabled
...
def main():
    ...
    config_path = os.path.join(Path.cwd().parent, 'configs', 'config_unimodal_audio.yaml')
