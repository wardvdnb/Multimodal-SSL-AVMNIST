...
class MultiModalSimCLRModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        img1, spec1, img2, spec2 = batch
        img1 = img1.float()
        spec1 = spec1.float()
        img2 = img2.float()
        spec2 = spec2.float()

        # Random modality selection for each view
        # 0: image-image, 1: audio-audio, 2: image-audio, 3: audio-image
        mode = torch.randint(0, 4, (1,)).item()

        if mode == 0:
            z1 = self.image_projection_head(self.image_encoder(img1, spec1))
            z2 = self.image_projection_head(self.image_encoder(img2, spec2))
        elif mode == 1:
            z1 = self.audio_projection_head(self.audio_encoder(img1, spec1))
            z2 = self.audio_projection_head(self.audio_encoder(img2, spec2))
        elif mode == 2:
            z1 = self.image_projection_head(self.image_encoder(img1, spec1))
            z2 = self.audio_projection_head(self.audio_encoder(img2, spec2))
        else:
...
class MultiModalSimCLRLightning(pl.LightningModule):
    def __init__(
        self,
        projection_dim=256,
        output_dim=256,
        ...
        self.model = MultiModalSimCLRModel(output_dim=output_dim, projection_dim=projection_dim)
